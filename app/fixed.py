...
base.APP_VERSION = "render-light-1.0.10"
...
def _reminder_date_from_text(message: str, history: list[dict]) -> tuple[datetime | None, str]:
    ...
    before = re.search(r"(?:dos|2)\s+semanas\s+antes\s+de(?:l)?\s+(\d{1,2})\s+de\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)(?:\s+de\s+(\d{4}))?", lowered, re.IGNORECASE)
    if before:
...
def _is_memory_or_reminder(message: str) -> bool:
    ...
    return any(word in lowered for word in ("recuerda", "recuérdame", "recuerdame", "recordatorio", "recordar", "solo recuerda", "guarda esa fecha"))


def _save_message(owner: str, role: str, content: str) -> None:
    with sqlite3.connect(base.DB_PATH, timeout=5) as db:
        db.execute("INSERT INTO messages(owner_hash, role, content) VALUES(?, ?, ?)", (owner, role, content))
...
async def ai_rx_cloud_guard(request, call_next):
    ...
    if _is_memory_or_reminder(message):
        ...
        if remind_date:
            ...
            reply = f"Desde luego, señor. He guardado en mi memoria el recordatorio para el {_format_spanish_date(remind_date)}. Lo tendré presente dentro de AI_RX."
        else:
    ...
    if reply is None:
        ...
        try:
            ...
            reply = "Perdón, señor: mi motor de respuesta en la nube tuvo un tropiezo momentáneo. Aun así sigo aquí; si era algo para recordar, escríbeme 'recuerda que...' y lo guardaré sin depender de Gemini."
    _save_message(owner, "user", message)
    _save_message(owner, "assistant", reply)
    return base.JSONResponse({"reply": reply, "provider": "gemini-local-guard", "model": settings.gemini_model, "memory_saved": memory_saved})

PHONE_CONTROL_JS = r'''
function aiRxNorm(text){ return String(text||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim(); }
function aiRxPhoneAvailable(){ return !!(window.AIRX && (typeof window.AIRX.runPhoneAction === 'function' || typeof window.AIRX.openClock === 'function')); }
function aiRxRunPhone(action, payload){
  if(!window.AIRX) return false;
  payload = payload || {};
  try{
    if(action === 'calendar' && typeof AIRX.openCalendar === 'function'){ AIRX.openCalendar(); return true; }
    if(action === 'clock' && typeof AIRX.openClock === 'function'){ AIRX.openClock(); return true; }
    if(action === 'photos' && typeof AIRX.openPhotos === 'function'){ AIRX.openPhotos(); return true; }
    if(action === 'camera' && typeof AIRX.openCamera === 'function'){ AIRX.openCamera(); return true; }
    if(action === 'contacts' && typeof AIRX.openContacts === 'function'){ AIRX.openContacts(); return true; }
    if(action === 'settings' && typeof AIRX.openSettings === 'function'){ AIRX.openSettings(); return true; }
    if(action === 'wifi' && typeof AIRX.openWifi === 'function'){ AIRX.openWifi(); return true; }
    if(action === 'phone' && payload.number && typeof AIRX.dialNumber === 'function'){ AIRX.dialNumber(String(payload.number)); return true; }
    if(action === 'phone' && typeof AIRX.openPhone === 'function'){ AIRX.openPhone(); return true; }
    if(action === 'sms' && typeof AIRX.composeSms === 'function'){ AIRX.composeSms(String(payload.number||''), String(payload.text||'')); return true; }
    if(action === 'set_alarm' && typeof AIRX.setAlarm === 'function'){ AIRX.setAlarm(Number(payload.hour||0), Number(payload.minute||0)); return true; }
    if(typeof AIRX.runPhoneAction === 'function'){ AIRX.runPhoneAction(action, JSON.stringify(payload)); return true; }
...
function aiRxHandlePhoneCommand(message){
  ...
  const openActions = [[/calendario|agenda/, 'calendar', 'Abriendo calendario, señor.'], [/reloj|alarmas/, 'clock', 'Abriendo reloj, señor.'], [/fotos|galeria|galería/, 'photos', 'Abriendo fotos, señor.'], [/camara|cámara/, 'camera', 'Abriendo cámara, señor.'], [/contactos/, 'contacts', 'Abriendo contactos, señor.'], [/wifi|wi-fi/, 'wifi', 'Abriendo ajustes de Wi‑Fi, señor.'], [/ajustes|configuracion|configuración/, 'settings', 'Abriendo ajustes, señor.'], [/telefono|teléfono|marcador/, 'phone', 'Abriendo teléfono, señor.']];
...
base.HTML = base.HTML.replace("#main { display:none; flex:1; min-height:0; }", "#main { display:none; flex:1; flex-direction:column; min-height:0; height:100%; max-height:calc(100svh - 20px); overflow:hidden; }")
base.HTML = base.HTML.replace("#chat { flex:1; overflow:auto; padding:14px; display:flex; flex-direction:column; gap:12px; min-height:45svh; }", "#chat { flex:1 1 auto; overflow:auto; padding:20px; display:flex; flex-direction:column; gap:18px; min-height:0; font-size:20px; }")
base.HTML = base.HTML.replace("form { display:flex; gap:10px; padding:12px; }", "form { display:flex; gap:14px; padding:16px 18px; align-items:flex-end; flex:none; }")
base.HTML = base.HTML.replace("textarea { resize:none; min-height:54px; max-height:140px; }", "textarea { resize:none; min-height:74px; max-height:170px; flex:1 1 auto; font-size:20px; border-radius:18px; }")
base.HTML = base.HTML.replace("form button { width:92px; flex:none; }", "form button { width:124px; flex:0 0 124px; min-height:74px; align-self:flex-end; font-size:18px; border-radius:18px; }")
base.HTML = base.HTML.replace("$('main').style.display='flex'; health();", "$('main').style.display='flex'; $('main').style.flexDirection='column'; health();")
base.HTML = base.HTML.replace("if(!r.ok) throw new Error(j.detail||'No autorizado');", "if(!r.ok){ const msg = typeof j.detail === 'string' ? j.detail : 'Escribe un código autorizado.'; throw new Error(msg||'No autorizado'); }")
base.HTML = base.HTML.replace("if(!r.ok) throw new Error(j.detail||'Error'); typing.remove(); add('bot',j.reply);", "if(!r.ok){ const msg = typeof j.detail === 'string' ? j.detail : 'No pude completar la petición.'; throw new Error(msg||'Error'); } typing.remove(); add('bot',j.reply);")
base.HTML = base.HTML.replace("let token = localStorage.getItem('ai_rx_token') || '';\nlet history = [];", "let token = localStorage.getItem('ai_rx_token') || '';\nlet history = [];\n" + PHONE_CONTROL_JS)
base.HTML = base.HTML.replace("add('user',msg); const typing=document.createElement('div');", "add('user',msg); if(aiRxHandlePhoneCommand(msg)) return; const typing=document.createElement('div');")
base.HTML = base.HTML.replace("@media (max-width:640px) { .app{padding:10px;} #login{min-height:calc(100svh - 20px); display:flex; flex-direction:column; justify-content:center;} form{position:sticky;bottom:0;background:rgba(6,9,20,.94);backdrop-filter:blur(16px);} .msg{max-width:94%;} }", "@media (max-width:640px) { body{font-size:20px !important;} .app{padding:8px;min-height:100svh;width:100%;} .card{border-radius:26px;} .logo{width:76px;height:76px;font-size:38px;border-radius:22px;} h1{font-size:38px;line-height:1.05;} p{font-size:18px;line-height:1.35;} .tiny,.status{font-size:16px;line-height:1.25;} input{font-size:21px;min-height:70px;padding:18px;border-radius:20px;} button{font-size:19px;min-height:64px;border-radius:20px;padding:16px 18px;} #login{width:min(620px,100%);min-height:calc(100svh - 16px);padding:38px;display:flex;flex-direction:column;justify-content:center;} .row{gap:14px;margin-top:14px;} #enter{margin-top:14px !important;} #main{min-height:calc(100svh - 16px);max-height:calc(100svh - 16px);} header{flex:none;padding:20px;} header strong{font-size:22px;} #chat{font-size:20px;padding:20px;gap:18px;} .msg{max-width:95%;font-size:20px;line-height:1.5;padding:16px 18px;border-radius:22px;} form{position:sticky;bottom:0;background:rgba(6,9,20,.96);backdrop-filter:blur(16px);padding:18px;} textarea{font-size:20px !important;min-height:76px;} form button{font-size:19px;min-height:76px;width:128px;flex-basis:128px;} }")
